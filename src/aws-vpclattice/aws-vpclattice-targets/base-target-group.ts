...
export abstract class TargetGroupBase extends core.Resource implements ITargetGroup {
  ...
  protected validateHealthCheck(healthCheck: HealthCheck): string[] {
    const errors: string[] = [];
    if (
      healthCheck?.healthyThresholdCount !== undefined &&
      (healthCheck.healthyThresholdCount < 1 || healthCheck.healthyThresholdCount > 10)
    ) {
      errors.push(
    ...
    if (
      healthCheck?.unhealthyThresholdCount !== undefined &&
      (healthCheck.unhealthyThresholdCount < 2 || healthCheck.unhealthyThresholdCount > 10)
    ) {
