...
import { HealthCheckProtocol, HealthCheckProtocolVersion, type HealthCheck } from './health-check';
...
export class IpTargetGroup extends TargetGroupBase {
  ...
  constructor(scope: constructs.Construct, id: string, props: IpTargetGroupProps) {
    ...
    this._resource = new aws_vpclattice.CfnTargetGroup(this, 'Resource', {
      ...
      targets: Lazy.any({
        produce: () =>
          this.targets.map(target => ({ id: target.ipAddress.toString(), port: target.port ?? this.port })),
      }),
      config: {
