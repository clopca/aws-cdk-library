 ...
 */
export class AuthPolicyStatement extends PolicyStatement {
  /**
  ...
  public static allowOnlyOrganization(orgId: string, resources?: string[]): AuthPolicyStatement {
    ...
    // }
    statement.addPrincipals(new StarPrincipal());
   ...
   * @returns `AuthPolicyStatement`
   */
  public static allowOnlyRole(role: IRole, resources?: string[]): AuthPolicyStatement {
    const statement = AuthPolicyStatement.buildBaseStatement();
    statement.addResources(...(resources ?? ['*']));
...
export class AuthPolicyDocument extends PolicyDocument {
   ...
   * @see https://docs.aws.amazon.com/AmazonS3/latest/API/sig-v4-authenticating-requests.html
   */
  public static readonly AUTHENTICATED_ONLY = new AuthPolicyDocument({
   ...
   * to the service network.
   */
  public static readonly UNAUTHENTICATED = new AuthPolicyDocument({
    ...
    if (!this.isEmpty) {
      ...
      for (const statement of policyJson.Statement) {
        ...
        if (!this.validateActions(statement.Action)) {
          errors.push(
            `Invalid action detected. Allowed actions for VPC Lattice are: ${validActions.join(', ')} or '*'.`,
          );
        }
      ...
      if (errors.length > 0) {
        errors.unshift(
          `The following errors were found in the VPC Lattice auth policy:${EOL}${JSON.stringify(policyJson, null, 2)}`,
        );
      }
    }
    return errors;
