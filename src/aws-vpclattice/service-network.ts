import { type aws_ec2 as ec2, aws_iam as iam, aws_ram as ram } from 'aws-cdk-lib';
    ...
    class Import extends ServiceNetworkBase {
      ...
      private validateServiceNetworkArn() {
        ...
        if (!arnPattern.test(arn)) {
          errors.push(
            `Service network ARN should be in the format 'arn:aws:vpc-lattice:<REGION>:<ACCOUNT>:servicenetwork/<NAME>', got ${arn}.`,
          );
        }
        return errors;
      }
