...
import { HealthCheckProtocolVersion, HealthCheckProtocol, type HealthCheck } from './health-check';
...
export class InstanceTargetGroup extends TargetGroupBase {
  ...
  constructor(scope: Construct, id: string, props: InstanceTargetGroupProps) {
    ...
    this.protocolVersion =
      props.protocolVersion ?? (props.protocol !== RequestProtocol.TCP ? RequestProtocolVersion.HTTP1 : undefined);
    ...
    this._resource = new aws_vpclattice.CfnTargetGroup(this, 'Resource', {
      ...
      targets: Lazy.any({
        produce: () =>
          this.instances.map(target => ({ id: target.instance.instanceId, port: target.port ?? this.port })),
      }),
      config,
    });
